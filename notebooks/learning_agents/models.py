...
class ActorCriticAgent(torch.nn.Module):
    def __init__(self, config:dict, num_players, obs_size=None):
        ...
        # hidden layers
        self.fc_joint = self._layer_init(torch.nn.Linear(config['rec_hidden_size'], config['joint_mlp_size']))
        self.policy_hidden = self._layer_init(torch.nn.Linear(config['joint_mlp_size'], config['split_mlp_size']))
        self.value_hidden = self._layer_init(torch.nn.Linear(config['joint_mlp_size'], config['split_mlp_size']))

        # policy output
        self.policies_out = torch.nn.ModuleList()
        for _ in range(config["num_votes"]):
            actor_branch = self._layer_init(torch.nn.Linear(in_features=config['split_mlp_size'], 
                                                            out_features=config['approval_states']),
                                                            std=0.01)
            self.policies_out.append(actor_branch)

        # value output
        self.value_out = self._layer_init(torch.nn.Linear(config['split_mlp_size'], 1), std=1.0)
